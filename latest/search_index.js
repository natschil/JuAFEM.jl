...

{
    ...
    "category": "section",
    "text": "In JuAFEM boundary Dirichlet boundary conditions are handled by DirichletBoundaryConditions.dbcs = DirichletBoundaryConditions(dh)Next we need to add contraints to dbcs. For this problem we define homogeneous Dirichlet boundary conditions on the whole boundary, i.e. the union of all the face sets on the boundary.∂Ω = union(getfaceset.(grid, [\"left\", \"right\", \"top\", \"bottom\"])...)Now we are set up to define our constraint. We specify which field the condition is for, and our combined face set ∂Ω. The last argument is a function which takes the spatial coordinate x and the current time t and returns the prescribed value. In this case it is trivial – no matter what x and t we return 0. When we have specified our contraint we add! it to dbcs.dbc = DirichletBoundaryCondition(:u, ∂Ω, (x, t) -> 0)\nadd!(dbcs, dbc)We also need to close! and update! our boundary conditions. When we call close! the dofs which will be constrained by the boundary conditions are calculated and stored in our dbcs object. Since the boundary conditions are, in this case, independent of time we can update! them directly with e.g. t = 0.close!(dbcs)\nupdate!(dbcs, 0.0)"
},
